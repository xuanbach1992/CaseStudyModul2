...
class CartController extends Controller
{
    ...
    public function plusProductIntoCart($productId)
    {
        if (Session::has('cart')) {
            $oldCart = Session::get('cart');
            $cart = new Cart($oldCart);
        ...
        return redirect()->route('product.showCard');
    }

    public function subProductIntoCart($productId)
    {
        if (Session::has('cart')) {
            $oldCart = Session::get('cart');
            $cart = new Cart($oldCart);
            $cart->subtraction($productId);
            Session::put('cart', $cart);
            Session::flash('success', 'Cập nhật thành công!');
        } else {
            Session::flash('delete_error', 'Bạn chưa mua sản phẩm nào');
